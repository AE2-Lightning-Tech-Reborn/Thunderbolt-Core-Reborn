...
class CpSatExecutionBlocksTest {
    ...
    @Test void jointlyChoosesSeedProductionAndTrillionRepetitions() {
        ...
        assertEquals(1L, result.plan().firings().get(graph.patternsFor("A").getFirst()));
        assertEquals(q, result.plan().firings().get(graph.patternsFor("B").getFirst()));
        assertEquals(q, result.plan().firings().get(graph.patternsFor("T").getFirst()));
    }
    ...
    @Test void reverseSeedCanProduceTheFirstOutputWithoutConsumingFuel() {
        ...
        assertEquals(9L, plan.firings().get(graph.patternsFor("B").getFirst()));
    }
