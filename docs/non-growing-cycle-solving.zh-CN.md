...
V2 路径中，原始候选图中的其他环仍由 `buildDag/frameFor` 删除回边配方。`CycleAnalysis.LOSSY_CONVERSION` 的方向重试甚至包含有增益的转换比例，因为每个尝试只保留无环子集，不会执行完整增益环。因此不能把当前所有“能做切环重试”的环，都解释为原始完整 SCC 已通过非自增证明。
...
V2 切环路径报告缺 `B=3`；在仓库现有整数求解器中保留完整原始配方，模型直接得到执行次数 `[1,2,1]`，独立逐批执行通过。这说明完整环进入整数模型有实际收益，继续增加切环方向数不能覆盖所有这种用法。
...
这些资料已核查，没有安装或嵌入上述外部工具。最初的实例原型使用 `BoundedIntegerLinearSolver`；当前独立 CP-SAT 生产路径已经保留准入的完整 SCC，并接入执行验证与证明驱动的细化，见 [CP-SAT 与 Petri 执行验证的闭环](cp-sat-petri-refinement.zh-CN.md)。余额成立本身仍不能直接返回可行。
