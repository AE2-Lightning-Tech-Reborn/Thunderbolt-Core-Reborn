...
| 方向 | 本实验实现 | 兼容性边界 |
| --- | --- | --- |
| AEItemKey / AEFluidKey | 各 4096 槽有界构造缓存；普通键和重复组件快照均可复用 | 保留原生 stack.copy、ItemLike.asItem、计数、popTime、完整 matches 及物品动态容量／耐久检查 |
| 带组件的键 | 使用 1.21 的写时复制 patch 身份索引，命中后仍检查完整内容 | 首次复制、可写或未知组件映射不参与缓存；独立但内容相同的快照仍按值相等 |
...
上游 1.21.1 未包含 1.20 的模型、Farmer's Delight 等优化，不能按首页功能数推算移植覆盖率。登录时强制 GC／等待也未引入。
...
物品类型、计数、动画状态或快照不同均不能直接复用；命中仍调用 AE2 的 matches 并核对构造器会保存的动态元数据。首次复制的快照不计算 identity hash，避免大量一次性组件负载的缓存成本。一次组件变更后，首次转换会回到原生构造。

并发首次访问、槽冲突和淘汰允许产生不同的等值对象。AE2 的 equals/hashCode、网络和存档入口不被改写。缓存清空采用换代；清空前正在构造的线程只能写入旧表。所有表有固定槽数，服务端关闭、配置重载／卸载会清理。
...
构造和复制操作使用可组合的 WrapOperation；若其他附属移除了对应分配点，`require = 0` 允许该项优化不生效。没有取消 AE2 工厂外层的 HEAD/RETURN、网络、存档或样板钩子。
...
完整 JUnit 回归为 835 项，失败／错误／跳过均为 0。运行测试源集不进入发布 jar。

边界：这不是七个附属所有机器、CPU 调度、客户端 GUI 和长期整包运行的穷举证明。精简测试包中仍有可选配方缺少 AE2WTLib／AE2CS 的加载日志、LT 的旧掉落表引用，以及 Useless 2.3.4 指向旧 ECO 类名的警告；已用未加入本实验优化的原始 Thunderbolt 源码在同一附属组合下启动对照服务端，确认这些日志同样存在；它们不是本次移植新增的问题，但不能用“17 项通过”宣称整包日志无异常。
